...
PropList[KiraPropagator[q2, mw], KiraPropagator[-p1 + q2, mw], 
 ...
 (((-2 + d)*EL^5*gWlN*gWNl*gWWA*gXXWW*s*(-2*bb*psq^2 + 
     aa*psq*(2*mm^2 + 2*psq - s) + bb*mm^2*(-2*psq + s))*\[Mu]^(8 - 2*d))/
   (2^(2*(1 + d))*Pi^(2*d)) + (2^(-1 - 2*d)*(-2 + d)*EL^5*gWlN*gWNl*gWWA*
    gXXWW*(-(aa*(psq*(2*psq - s) + mm^2*(2*psq + s))) + 
     bb*(mm^2*(2*psq - s) + psq*(2*psq + s)))*\[Mu]^(8 - 2*d)*
    SPList[SP[p1, q2]])/Pi^(2*d) + (2^(-1 - 2*d)*(aa - bb)*(-2 + d)*EL^5*gWlN*
    gWNl*gWWA*gXXWW*(mm^2 + psq)*s*\[Mu]^(8 - 2*d)*SPList[SP[p2, q2]])/
   Pi^(2*d) + ((-2 + d)*EL^5*gWlN*gWNl*gWWA*gXXWW*(aa*mm^2 - bb*psq)*
