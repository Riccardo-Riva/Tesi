(* Created with the Wolfram Language : www.wolfram.com *)
PropList[KiraPropagator[q1, 0], KiraPropagator[p3 + q1, mw], 
  KiraPropagator[-p1 - p2 + p3 + q1, mw]]*
 (-((2^(-1 - d)*(-2 + d)*EL^3*gWlN*gWNl*gWWA*(bb*(psq - s - t) + 
      aa*(2*mm^2 - 3*psq + s + t))*\[Mu]^(4 - d)*SPList[SP[p1, q1], 
      SP[p1, q1]])/Pi^d) - (2^(-1 - d)*(-2 + d)*EL^3*gWlN*gWNl*gWWA*
    (bb*(2*mm^2 - s - 2*t) + aa*(2*mm^2 - 4*psq + s + 2*t))*\[Mu]^(4 - d)*
    SPList[SP[p1, q1], SP[p2, q1]])/Pi^d - 
  (2^(-1 - d)*(aa - bb)*(-2 + d)*EL^3*gWlN*gWNl*gWWA*(2*psq - s - 2*t)*
    \[Mu]^(4 - d)*SPList[SP[p1, q1], SP[p3, q1]])/Pi^d + 
  (2^(-1 - d)*(-2 + d)*EL^3*gWlN*gWNl*gWWA*(aa*(psq - t) + 
     bb*(-2*mm^2 + psq + t))*\[Mu]^(4 - d)*SPList[SP[p2, q1], SP[p2, q1]])/
   Pi^d - (2^(-1 - d)*(aa - bb)*(-2 + d)*EL^3*gWlN*gWNl*gWWA*
    (2*psq - s - 2*t)*\[Mu]^(4 - d)*SPList[SP[p2, q1], SP[p3, q1]])/Pi^d)
