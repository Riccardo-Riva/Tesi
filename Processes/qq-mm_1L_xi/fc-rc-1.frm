...
s Alfa, Alfa2, Alfas, Alfas2, CA, CA2, CB, CB2, CBA, CBA2, cG3, cG4,
  CW, CW2, d, D, EL, GS, MA0, MA02, MA0tree, MA0tree2, MB, MB2, MC,
  MC2, MD, MD2, ME, ME2, MGl, MGl2, MH, Mh0, Mh02, Mh0tree, Mh0tree2,
  MH2, MHH, MHH2, MHHtree, MHHtree2, MHp, MHp2, MHptree, MHptree2,
  Mino3, Mino3C, ML, ML2, MM, MM2, MS, MS2, MT, MT2, MU, MU2, MW, MW2,
  MZ, MZ2, pG1, pG2, Pi, pM1, pM2, SA, SA2, SB, SB2, SBA, SBA2,
...
f Spinor;
d D;
i Col1 = 0, Col2 = 0, iM = 0, Lor1, Lor2;
t Pol;
...
L T1G1 = -(intM(Den(Removed(q1), pM1^2), 
   Den(-k1 + Removed(q1), pM2^2))*
  ((cG4*g6_(100))/2 + (cG3*g7_(100))/2)*g_(100)*
   (pM1 + g_(100, Removed(q1)))*((pG2*g6_(100))/2 + 
    (pG1*g7_(100))/2)*powM(2, -d)*powM(Pi, 2 - d)*SUNSum(Col1)*
  SUNSum(Col2));

L T1G2 = d_(Lor1, Lor2)*intM(Den(Removed(q1), pM1^2), 
  Den(-k1 + Removed(q1), pM2^2))*
 (cG4*g_(100, Lor2)*(g6_(100)/2) + 
   cG3*g_(100, Lor2)*(g7_(100)/2))*g_(100)*
  (pM1 + g_(100, Removed(q1)))*
  (pG2*g_(100, Lor1)*(g6_(100)/2) + 
   pG1*g_(100, Lor1)*(g7_(100)/2))*powM(2, -d)*powM(Pi, 2 - d)*
 SUNSum(Col1)*SUNSum(Col2);


#define Legs "2"
#define Invariants ""
#define OnShell "False"
#define MomElim "False"
#define MomRange "1,2"
#define DotExpand "0"
...
ab Alfa, Alfa2, Alfas, Alfas2, CA, CA2, CB, CB2, CBA, CBA2, CW, CW2, d,
...
L iT1G1 = T1G1*(replace_(pM1, MD, pM2, MW, pG1, 
   -(i_*EL*MD*root_(2, 2))/(2*MW*SW), pG2, 
   (i_*EL*MU*root_(2, 2))/(2*MW*SW), cG3, 
   (i_*EL*MU*root_(2, 2)*SUNT(Col1, Col2))/(2*MW*SW), cG4, 
   -(i_*EL*MD*root_(2, 2)*SUNT(Col1, Col2))/(2*MW*SW)) + 
  replace_(pM1, MU, pM2, MH, pG1, -(i_*EL*MU)/(2*MW*SW), pG2, 
   -(i_*EL*MU)/(2*MW*SW), cG3, -(i_*EL*MU*SUNT(Col1, Col2))/
    (2*MW*SW), cG4, -(i_*EL*MU*SUNT(Col1, Col2))/(2*MW*SW)) + 
  replace_(pM1, MU, pM2, MZ, pG1, (EL*MU)/(2*MW*SW), pG2, 
   -(EL*MU)/(2*MW*SW), cG3, (EL*MU*SUNT(Col1, Col2))/(2*MW*SW), cG4, 
   -(EL*MU*SUNT(Col1, Col2))/(2*MW*SW)));

L iT1G2 = T1G2*(replace_(pM1, MD, pM2, MW, pG1, 
   (i_*EL*root_(2, 2))/(2*SW), pG2, 0, cG3, 
   (i_*EL*root_(2, 2)*SUNT(Col1, Col2))/(2*SW), cG4, 0) + 
  replace_(pM1, MU, pM2, 0, pG1, (-2*i_*EL)/3, pG2, (-2*i_*EL)/3, 
   cG3, (-2*i_*EL*SUNT(Col1, Col2))/3, cG4, 
   (-2*i_*EL*SUNT(Col1, Col2))/3) + replace_(pM1, MU, pM2, MZ, pG1, 
   (i_*EL*(1/2 - (2*SW^2)/3))/(CW*SW), pG2, (-2*i_*EL*SW)/(3*CW), 
   cG3, (i_*EL*(1/2 - (2*SW^2)/3)*SUNT(Col1, Col2))/(CW*SW), cG4, 
   (-2*i_*EL*SW*SUNT(Col1, Col2))/(3*CW)));

.sort
drop;

L Result = iT1G1 + iT1G2;
